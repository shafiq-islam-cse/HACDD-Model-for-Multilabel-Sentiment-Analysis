...
def load_glove(word_index):
    EMBEDDING_FILE = 'C:/Users/Shafiq-AP-CSE-MIST/Desktop\KHACDD QC/EMBEDDING/glove.840B.300d.txt'
...
def load_fasttext(word_index):
    EMBEDDING_FILE = 'C:/Users/Shafiq-AP-CSE-MIST/Desktop\KHACDD QC/EMBEDDING/wiki-news-300d-1M.vec'
...
def load_para(word_index):
    EMBEDDING_FILE = '/EMBEDDING/paragram_300_sl999.txt'
    def get_coefs(word,*arr): return word, np.asarray(arr, dtype='float32')
...
df_train = pd.read_csv("IMDBmodified.csv")
df_test = pd.read_csv("IMDBmodified_test.csv")
...
sin = len(df_train[df_train["sentiment"]==0])
insin = len(df_train[df_train["sentiment"]==1])
...
def add_features(df):
    ...
    df[review] = df[review].progress_apply(lambda x:str(x))
    df['total_length'] = df[review].progress_apply(len)
    df['capitals'] = df[review].progress_apply(lambda comment: sum(1 for c in comment if c.isupper()))
    df['caps_vs_length'] = df.progress_apply(lambda row: float(row['capitals'])/float(row['total_length']),
                                axis=1)
    df['num_words'] = df.text.str.count('\S+')
    df['num_unique_words'] = df[review].progress_apply(lambda comment: len(set(w for w in comment.split())))
    df['words_vs_unique'] = df['num_unique_words'] / df['num_words']

    return df

def load_and_prec():
    train_df = pd.read_csv("ERmodified.csv")
    test_df = pd.read_csv("ERmodified_test.csv")
...
class MyDataset(Dataset):
    ...
    def __getitem__(self, index):
        data, sentiment = self.dataset[index]

        return data, sentiment, index
    def __len__(self):
